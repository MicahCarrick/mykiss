...
class Window(Gtk.Window, LogObject):
    ...
    def add_widget(self, name, widget, icon_name=icons.STOCK_PLUGIN, 
                   orientation=Gtk.Orientation.VERTICAL):
        """
        Add a new widget to the window. The initial placement can be suggested
        with the `orientation` argument, however, the user ultimately has control
        over the dock layout which is loaded and saved at each session.
        
        Args:
            name       -- The name for the dock item
        ...
        """
        if not '_widgets' in self:
            self._widgets = {}
            
        # create the dock item
        item = Gdl.DockItem.new_with_stock(name, " "+name,
                                           icon_name,
        ...
        menu_item = Gtk.ImageMenuItem(label=name)
        ...
        # store info in the widget
        widget.dock_item = item
        widget.menu_item = menu_item
    ...
    def _init_plugin_manager(self):
        ...
        box.pack_start(plugin_list, True, True, 0)    
        box.show_all()
        self.add_widget("Plugin Manager", box, icons.STOCK_PLUGIN, Gtk.Orientation.VERTICAL)
