import sys
import os
import glob
import imp

from gi.repository import GLib
from gi.repository import GObject

from gettext import gettext as _

from logobject import LogObject
import icons

default_plugin_engine = None

class ValidationError(Exception):
    """
    Exception class for validation errors in plugin info files.
    
    Args:
        msg: The error message.
        filename: The name of the invalid file. 
    """
    def __init__(self, msg, filename=None):
        if filename:
            Exception.__init__(self, "%s in %s" % (msg, filename))
        else:
            Exception.__init__(self, msg)

class PluginInfo(object):
    """
    Information about a plugin as defined in a key/value file.
    
    Args:
        plugin_file: See parse_plugin_file()
    
    Raises:
        ValidationError: The plugin key/value file is invalid.
    """
    def __init__(self, plugin_file):
        self._required_keys = ('name', 'version', 'activate', 'description')
        if plugin_file:
            self.parse_plugin_file(plugin_file)
    
    def parse_plugin_file(self, plugin_file):
        """
        Parse the plugin information file and map the key names and values to
        class attributes. 
        
        Arguments:
            plugin_file: Full path to the plugin info key/value file.
        
        Raises:
            ValidationError: The plugin key/value file is invalid.
        """
        self.plugin_file = plugin_file
        self.short_name = os.path.basename(plugin_file)[0:-7]
        self.stock_id = icons.STOCK_PLUGIN # overriden if icons are provided
        # TODO: KeyFile is broken in GLib 2.30. Use config parser or ini file (xdg)
        parser = GLib.KeyFile()
        parser.load_from_file(plugin_file, GLib.KeyFileFlags.NONE)
        if not parser.has_group("Plugin"):
            raise ValidationError(_("Missing 'Plugin' group."))
        for key in parser.get_keys("Plugin")[0]:
            setattr(self, key, parser.get_string("Plugin", key))
        for key in self._required_keys:
            try:
                getattr(self, key)
            except AttributeError:
                raise ValidationError(_("Missing required key: '%s'") % key,
                                      plugin_file)
        # make activate an absolute path
        self.activate = os.path.join(os.path.dirname(self.plugin_file), self.activate)
        self.sys_path = os.path.dirname(self.activate)
             
        # convert version to a list of integers for easy version comparison
        self.version_string = self.version
        try:
            self.version = [int(value) for value in self.version.split(".")]
            self.version = (self.version + [0] * 3)[:3] # pad with 0
        except ValueError:
            raise ValidationError(_("Invalid format for key: 'version'"), plugin_file)

class PluginEngine(GObject.Object, LogObject):
    """
    Plugin Engine
    
    The MyKiss plugin engine.
    """
    __gtype_name__ = 'PluginEngine'
    __gsignals__ = {
        "plugin-enabled": 
            (GObject.SIGNAL_RUN_FIRST, GObject.TYPE_NONE, 
            (GObject.TYPE_PYOBJECT,)),
        "plugin-disabled": 
            (GObject.SIGNAL_RUN_FIRST, GObject.TYPE_NONE, 
            (GObject.TYPE_PYOBJECT,)),
    }
    def __init__(self):
        GObject.Object.__init__(self)
        self.init_logging()
        self._plugin_dirs = []
        self._enabled_plugins = []
        self._loaded_modules = []
    
    @GObject.property
    def plugin_dirs(self):
        return self._plugin_dirs
    
    @GObject.property
    def enabled_plugins(self):
        return self._enabled_plugins
    
    @GObject.property
    def installed_plugins(self):
        return self._installed_plugins
    
    def activate(self, plugin_class, attributes):
        m = __import__("mykiss.plugins", globals(), locals(), [plugin_class,], -1)
        klass = getattr(m, plugin_class)
        for plugin_class in klass.__subclasses__():
            plugin = plugin_class()
            if plugin.__module__ in self._loaded_modules:
                for attribute, value in attributes:
                    setattr(plugin, attribute, value)
                plugin.do_activate()
        """
        name = "terminal"
        activation = "WindowPlugin"
        info = __import__("plugins."+name, globals(), locals(), ['name', 'module'], -1)
        print info.name
        module = __import__("plugins."+name+"."+info.module, globals(), locals(), ['Plugin'], -1)
        from plugin import WindowPlugin
        for plugin_class in WindowPlugin.__subclasses__():
            plugin = plugin_class()
            plugin.window = self
            plugin.do_activate()
        """
        
    def add_plugin_dir(self, path):
        """
        Add 'path' to the list of directories to scan for plugins and also add
        to sys.path so that the plugin(s) can be imported.
        
        Args:
            path: Full path to the directory containing plugins. 
        """
        if path not in sys.path:
            sys.path.append(path)
            
        if path not in self._plugin_dirs:
            self._plugin_dirs.append(path)
    
    def enable(self, plugin_info):
        """
        Add the plugin to the list of enabled plugins and emit the 
        "plugin-enabled" signal.
        """
        self.emit("plugin-enabled", plugin_info)
        if not plugin_info in self.installed_plugins:
            self._enabled_plugins.append(plugin_info)
            # TODO: move to _load_plugin()/_unload_plugin()
            imported = False
            try:
                """
                module = "mykissplugins_%s" % plugin_info.short_name
                print os.path.dirname(plugin_info.activate)
                sys.path.append(plugin_info.sys_path)
                imp.load_source(module, plugin_info.activate)
                from test import test_support
                """
                #module = plugin_info.module.rpartition(".")
                __import__(plugin_info.module, globals(), locals(), [], -1)
            except Exception, e:
                self.warning("Cannot import %s: %s" % (plugin_info.module, str(e)))
            #print imported
            if plugin_info.module in sys.modules:
                # TODO: need to replace it if it's already present
                self._loaded_modules.append(plugin_info.module)
            else:
                self.warning("Plugin module was not loaded: %s" % plugin_info.module)
            #print self._loaded_modules
            #print sys.getrefcount(plugin_info.module)
            #print "testplugin.test" in sys.modules
    
    def disable(self, plugin_info):
        """
        Remove the plugin to the list of enabled plugins and emit the 
        "plugin-disabled" signal.
        """
        self.emit("plugin-disabled", plugin_info)
        # TODO: should be deactivated as well
        try:
            self._installed_plugins.remove(plugin_info)
        except ValueError:
            pass # plugin is already disabled
        
    def get_plugin_info(self, plugin_name):
        """
        Return a PluginInfo object for the plugin or None if the plugin file
        is not found in any of the "plugin_dirs". If the same plugin is found in 
        multiple directories, return a PluginInfo for the plugin the highest 
        version number.
        
        Args:
            plugin_name: The plugin name corresponding to the name of the 
                 .plugin file. Eg, "foobar.plugin" would be "foobar".
        """
        found_plugins = []
        for path in self._plugin_dirs:
            plugin_file = os.path.join(path, plugin_name + ".plugin")
            if os.path.exists(plugin_file):
                info = PluginInfo(plugin_file)
                found_plugins.append(info)
        if not found_plugins:
            return None
        elif len(found_plugins) == 1:
            return found_plugins[0]
        
        # version comparison
        plugin_info = found_plugins[0]
        for info in found_plugins:
            if info.version[0] > plugin_info.version[0]:
                plugin_info = info
            elif info.version[0] == plugin_info.version[0]:
                if info.version[1] > plugin_info.version[1]:
                    plugin_info = info
                elif info.version[1] == plugin_info.version[1]:
                    if info.version[2] > plugin_info.version[2]:
                        plugin_info = info
        return plugin_info
    
    def is_enabled(self, plugin_info):
        """
        Return True if the plugin specified is in the list of enabled plugins,
        False otherwise.
        
        Args:
            plugin_info: A PluginInfo instance. See get_plugin_info().
        """
        for info in self._enabled_plugins:
            if info.plugin_file == plugin_info.plugin_file:
                return True
        return False
   
    def register_stock_icons(self):
        """
        Register stock icons for each plugin which does not already provide
        a stock_id. The stock_id for the icons will have "mykiss-plugin-" 
        prefixed. The icons are registered based on a standard naming convention 
        for the plugin directory structure:
        
        /<plugin_name>/icons/<icon_size>/<plugin_name>.[png|svg]
        /<plugin_name>.plugin
        """
        for plugin in self.installed_plugins:
            if plugin.stock_id == icons.STOCK_PLUGIN:
                path = os.path.join(os.path.dirname(plugin.plugin_file), 
                                    plugin.short_name, "icons")
                if os.path.exists(path):
                    icons.register_stock_icons(path, (plugin.short_name,), 
                                               "mykiss-plugin-")
                    plugin.stock_id = "mykiss-plugin-" + plugin.short_name
         
    def set_enabled_plugins(self, plugin_names):
        """
        Set the list of enabled plugins. If a plugin that has already been 
        activated is not in the new list then it will be deactivated.
        
        Args:
            plugin_names: A list or tuple of plugin names corresponding to the
                name of the .plugin file. Eg, "foobar.plugin" would be "foobar".
        """
        for plugin in self.enabled_plugins:
            self.disable(plugin)
   
        self._enabled_plugins = []
        for name in plugin_names:
            try:
                info = self.get_plugin_info(name)
            except ValidationError, e:
                self.warning(e)
                continue
            if not info:
                self.warning(_("Plugin not found: %s"), name)
                continue
            self.enable(info)
    
    def scan_plugin_dirs(self):
        """
        Return a list of PluginInfo objects for every installed plugin. A plugin
        is 'installed' if it is found in any of the plugin directories passed
        to add_plugin_dir().
        """
        # first get each plugin's name from .plugin files. Let get_plugin_info()
        # determine which plugin version is the correct one.
        names = []
        for path in self._plugin_dirs:
            pattern = os.path.join(path, "*.plugin")
            for plugin_file in glob.glob(pattern):
                name = os.path.basename(plugin_file)[0:-7]
                if name not in names:
                    names.append(name)
        self._installed_plugins = []
        for name in names:
            try:
                self._installed_plugins.append(self.get_plugin_info(name))
            except ValidationError, e:
                self.warning(e)
                continue
        
def get_default_plugin_engine():
    """
    Return the default instance of the plugin engine.
    """
    global default_plugin_engine
    if not default_plugin_engine:
        default_plugin_engine = PluginEngine()
    return default_plugin_engine

"""
    name = "terminal"
    activation = "WindowPlugin"
    info = __import__("plugins."+name, globals(), locals(), ['name', 'module'], -1)
    print info.name
    module = __import__("plugins."+name+"."+info.module, globals(), locals(), ['Plugin'], -1)
    from plugin import WindowPlugin
    for plugin_class in WindowPlugin.__subclasses__():
        plugin = plugin_class()
        plugin.window = self
        plugin.do_activate()
"""
