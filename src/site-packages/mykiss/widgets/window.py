...
from mykiss.widgets import Statusbar
from mykiss import Document
from mykiss import icons

class Window(Gtk.ApplicationWindow, LogObject):
    """
    ...
    def __init__(self, application):
        ...
        self.set_title(application.package_name)
        ...
        box.pack_end(self._statusbar, False, False, 0)
        box.show_all()
        self.add(box)
        ...
        try:
            if config.getboolean('Window', 'restore_size'):
        ...
        # causes application to emit the "window-added" signal
        self.set_property("application", application)
    ...
    def _init_editor(self):
        ...
        self._editor.connect("active-document-info-changed", 
                             self._on_editor_active_document_info_changed)
    ...
    def _init_menu_and_toolbar(self, manager):
        ...
        self._document_actions.add_actions([
            ('Save', Gtk.STOCK_SAVE, None, None, None, 
                self._on_file_save_activate),
            ('SaveAs', Gtk.STOCK_SAVE_AS, _("Save As..."), None, None, 
                self._on_file_save_as_activate),
    ...
    def _on_file_open_activate(self, action, data=None):
        uri = self.open_file_dialog()
        if not uri:
            return
        document = Document(uri)
        self._editor.new_tab(document, True)
    ...
    def _on_editor_active_document_changed(self, editor, document, data=None):
        self._update_ui_for_document(document)
        
    def _on_editor_active_document_info_changed(self, editor, document, data=None):
        self._update_ui_for_document(document)
        
    def _update_ui_for_document(self, document):
        """
        Update titlebar, statusbar, actions, etc. for the current document.
        """
        self.set_title("%s - %s" % (document.name, self.application.package_name))
        self._statusbar.set_mime_type(document.get_mime_type())
        self._statusbar.set_language_name(document.get_language_name())
        self._statusbar.set_encoding(document.get_encoding())
        giofile = document.get_file()
        if giofile:
            self._statusbar.set_status(giofile.get_uri())
        else:
            self._statusbar.set_status(document.get_name())
    ...
    def _on_help_about_activate(self, action, data=None):
        self.application.show_about_dialog(self)
    ...
    def _on_show(self, window, data=None):  
        ...
        # make sure window has been sized
        while Gtk.events_pending(): 
            Gtk.main_iteration() 
        self.load_dock_layout(self._dock_layout_file)
    
    def tmp(self, window, data=None):
        print "size-allocate"
    ...
    def _restore_window_size(self):
        """ Restore the window size from the configuration file. """
        ...
        try:
            ...
            self.debug("Restoring window size: %dx%d", width, height)
    ...
    def _save_window_size(self):
        """ Save the window size to the configuration file. """
        if self.get_window().get_state() != 0:
            return # ignore if fullscreen, maximized, etc.
        size = self.get_size()
        #position = self.get_position()
        self.debug("Saving window size: %dx%d", size[0], size[1])
        config = self.application.config
