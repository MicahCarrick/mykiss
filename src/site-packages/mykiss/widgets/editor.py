...
class Editor(Gtk.Paned, LogObject):
    __gsignals__ = {
        "active-document-changed":      (GObject.SIGNAL_RUN_FIRST, 
                                         GObject.TYPE_NONE,
                                        (GObject.TYPE_PYOBJECT,)),
        "active-document-info-changed": (GObject.SIGNAL_RUN_FIRST, 
                                         GObject.TYPE_NONE,
                                        (GObject.TYPE_PYOBJECT,)),
        "document-added":               (GObject.SIGNAL_RUN_FIRST, 
                                         GObject.TYPE_NONE,
                                        (GObject.TYPE_PYOBJECT,)),
        "document-removed":             (GObject.SIGNAL_RUN_FIRST, 
                                         GObject.TYPE_NONE,
                                        (GObject.TYPE_PYOBJECT,)),
        "close-tab-clicked":            (GObject.SIGNAL_RUN_FIRST, 
                                         GObject.TYPE_NONE,
                                        (GObject.TYPE_PYOBJECT,)),
    }
    def __init__(self):
    ...
    def new_tab(self, document=None, set_active=False):
        ...
        tab.get_document().connect("info-changed", self._on_document_info_changed)
    ...
    def set_active_document(self, document):
        ...
        self.debug('"active-document-changed" %s', document.name)
    ...
    def _create_notebook(self):
        ...
        provider.load_from_data("GtkNotebook tab { padding-left: 1px; padding-right: 1px; }")
        ...
        notebook.connect("page-removed", self._on_notebook_page_removed)
    ...
    def _on_document_info_changed(self, document, data=None):
        """ Document info (icon, encoding, mime, etc.) has been updated. """
        if document == self._active_document:
            self.debug('"active-document-info-changed" %s', document.name)
            self.emit("active-document-info-changed", document)
    ...
    def _on_notebook_page_added(self, notebook, page, page_num, data=None):
        document = page.editor_tab.document
        self.debug('"document-added" %s', document.name)
        self.emit("document-added", document)
    
    def _on_notebook_page_removed(self, notebook, page, page_num, data=None):
        document = page.editor_tab.document
        self.debug('"document-removed" %s', document.name)
        self.emit("document-removed", document)
        
    def _on_tab_close_clicked(self, editor_tab, document, data=None):
        self.debug('"close-tab-clicked" %s', document.name)
