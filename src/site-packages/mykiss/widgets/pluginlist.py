...
class PluginList(Gtk.TreeView):
    ...
    __gsignals__ = {
        "plugin-selected": (GObject.SIGNAL_RUN_FIRST, GObject.TYPE_NONE, (GObject.TYPE_PYOBJECT,)),
    }  
    """
    ...
    def _on_cursor_changed(self, treeview, data=None):
        """ Emit the "plugin-selected" signal for the selected row. """
        selection = self.get_selection()
        model, iter = selection.get_selected()
        if iter:
            self.emit("plugin-selected", model[iter][5])
